...
const SelectEquipment = ({
  ...
  const handleSportSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setAvailableEquipment(sportsItemsMap.get(selectedSport));
  };
  ...
  const handleAddEquipment = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (
      equipmentList.some(
        (equipment) => equipment.name === value && equipment.sport === sport
      )
    ) {
      return;
    }
        ...
        <>
          <div className="flex justify-between space-x-2 items-center w-full">
            <div className="flex-row flex-1 text-center bg-green text-black py-2 px-2 rounded-md font-semibold">
              Selected: {sport.charAt(0).toUpperCase() + sport.slice(1)}
            </div>
            <button
          ...
          {equipmentList.map((equipment, index) => (
            <div
              key={index}
              className="flex justify-between space-x-2 items-center w-full"
            >
              <div className="flex-row flex-none items-center bg-white text-white py-2 px-2 rounded-md font-semibold"></div>
              <select
          ...
          <div className="flex justify-between space-x-2 items-center w-full">
            ...
            <select
              ...
              onChange={(e) => {
                if (getAvailableEquipment()?.length === 0) {
                  return;
                } else {
            ...
            >
              ...
              {availableEquipment?.map((equipment, i) => (
                <option value={equipment} key={i}>
                  {equipment}
                </option>
              ))}
              {/* {sportsItemsMap.get(sport)?.map((item, i) => (
